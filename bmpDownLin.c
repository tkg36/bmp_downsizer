...
void batchRun(char *inputBatchDir, char *outputBatchDir, float scale){
    ...
    if ((dfd = opendir(inputBatchDir)) == NULL)
    {
    ...
    while ((dp = readdir(dfd)) != NULL)
    {
        if(strstr(dp->d_name, ".bmp")!=NULL){
            char *filename_qfd, *combinedFileName,*combinedOutputFileName;
            filename_qfd =(char*)malloc(255*16);
            combinedFileName=(char*)malloc(255*16);
            combinedOutputFileName=(char*)malloc(255*16);
            struct stat *stbuf ;
            sprintf( filename_qfd , "%s/%s",inputBatchDir,dp->d_name) ;

            sprintf(combinedFileName,"%s/%s", inputBatchDir, dp->d_name) ;
            sprintf(combinedOutputFileName,"%s/%s", outputBatchDir, dp->d_name) ;
            downsizeBMP(combinedFileName, combinedOutputFileName, scale);

            free(filename_qfd);
            free(combinedFileName);
            free(combinedOutputFileName);
        }
